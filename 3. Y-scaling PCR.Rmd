...
---
# IODS-Final: Boston dataset Y-aware x transformation and PCG

Last we are trying something bit out of the scope of the course, but as it seemed it could solve my problem of dimensionality reduction I wanted to try it out. We will transform the x-variables using this [guide](https://github.com/WinVector/PreparingDataWorkshop/blob/master/YAwarePCA/YAwarePCA.Rmd). 


x’ such that a unit change in x’ corresponds to a unit change in y. Under this rescaling, all the independent variables are in the same units,which are indeed the natural units for the problem at hand: characterizing their effect on y. (We also center the transformed variables x’ to be zero mean, as is done with standard centering and scaling).
...
packages_to_load <- c("dplyr", "ggplot2", "reshape2","visreg", "tidyverse", "vtreat","WVPlots")
...
treatmentsN <- vtreat::designTreatmentsN(trainb,setdiff(colnames(trainb),'nox'),'nox',
                                 verbose=FALSE)
...
examplePruneSig = 1
trainbNTreatedYScaled <- vtreat::prepare(treatmentsN,trainb,pruneSig=examplePruneSig,scale=TRUE)
testbNTreatedYScaled <- vtreat::prepare(treatmentsN,trainb,pruneSig=examplePruneSig,scale=TRUE)
...
Here is shown few of the treated variables. Notice how the values became very low, but nox is still as it was before.
...
Here we show the contribution of the individual pricipal components.
...
We build two models one with three principal components and one with two. By AIC the one with two princibal components is selected for further analysis. We can explain **72%** of deviation.

The lower end is now explained better, but the upper end seems to be poor fit.
...
The model has lower dimensions and quite ok level of explained deviance.
...
```
I unfortunately run out of time, but the last model has the best explanary power and it is most parsimonious, so it is the best model.
